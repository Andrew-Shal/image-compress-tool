...
const { statSync, rmdir } = require("fs")
...
function getImagesInfo(filePaths) {
  ...
  function isValidCompressionQuality(val){ return val >= config.COMPRESSION_MIN && val <= config.COMPRESSION_MAX}
  
  function getUniqueUID(){ return uuidv4()}
  
  function getMbSize(size){ return `${(size / 1000000.0).toFixed(2)}mb`}
  function getKbSize(size){ return `${(size / 1000.0).toFixed(2)}kb`}
  
  function convertPrecentageToFloat(val){return parseFloat((val / 100).toFixed(2))}
